...
const Sidebar = () => {
  const { User,setUser,count } = useContext(UserContext);
  ...
  return (
    <div>
      <div className="bg-blue-300  top-16 bottom-0 fixed w-[16%] h-screen overflow-x-hidden">
        <form
          id="uploadform"
          hidden
          action="/fileupload"
        ...
        <div className="prodets flex flex-col items-center mt-5">
          <div className="relative">
            <span
              id="uploadicon"
              className="w-[30px] h-[30px] absolute bottom-0 right-0 rounded-full flex items-center justify-center bg-white"
          ...
          <div className="mx-6 text-gray -800 font-semibold ">My Posts:{count}</div>
          <div className="flex items-center gap-10 mt-2  px-3">
            { User && (
              <button onClick={createPostHandler} className="bg-white cursor-pointer rounded-lg mt-2 p-2 font-bold text-black">
                Add New
              </button>
              
            )}
          </div>
          <Link className="bg-gray-200 cursor-pointer rounded-lg font-bold mt-2 py-1 px-2" to="/feed">Posts</Link>
        </div>
      </div>
    </div>
  );
